 ...
 *
 */
public class AmaxReducer extends Reducer<Text, AreaAvgWritable, Text, FloatWritable> {
	...
	public void reduce(Text key, Iterable<AreaAvgWritable> value, Reducer<Text, AreaAvgWritable, Text, FloatWritable>.Context context)
		...
		this.maximum.set(sum.floatValue());
		String[] name = key.toString().split("/");
		this.outputKey.set(name[(name.length - 1)] + "  maximum:");
		context.write(this.outputKey, this.maximum);
	}
}